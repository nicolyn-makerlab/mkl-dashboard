...
Three manual lists in `lib/chat-summary.js` need upkeep:
...
- `SAFE_TITLE_CASE_PAIRS` — two-word business/product terms (e.g. "Trade Desk") that the full-name scrubber (below) would otherwise wrongly treat as a person's name and truncate. Add a term here if you see a legitimate phrase getting cut down to one word.
...
The very next real run after that decision proved prompt instructions alone aren't reliable enough even for the narrower "never a full name" rule: it produced a topic naming an employee's first and last name together. `scrubFullNames()` in `lib/chat-summary.js` is a code-level backstop for this — it drops the second word of any two consecutive Title Case words in `title`/`outcome` unless the pair is in `SAFE_TITLE_CASE_PAIRS`, the same "don't trust the model alone" principle already used for `deliveredBy`. It's a heuristic, not a guarantee — it can miss a name it doesn't recognize as a pair (e.g. one embedded differently in a sentence) and can over-redact an unlisted two-word phrase, which is the safer failure direction.
