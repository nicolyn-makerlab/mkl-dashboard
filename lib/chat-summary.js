...
const SUMMARY_PROMPT = `You're summarizing internal Google Chat messages about a marketing agency client account, for a short status panel on an internal dashboard. Read the messages below and produce short bullet points (each under 15 words) naming the general topics being discussed - one per distinct topic, however many that is.
...
async function main() {
  ...
  for (const [company, spaces] of Object.entries(COMPANY_SPACES)) {
    ...
    // TODO: this is a stopgap - listRecentMessages now returns structured
    // {text, threadName, senderDisplayName, ...} objects (for the
    // per-thread/per-outcome redesign in progress), but this function
    // still only consumes plain text pending chat-thread-check results.
    topicsByCompany[company] = await summarizeCompany(anthropic, messages.map((m) => m.text));
