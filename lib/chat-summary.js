...
// backstop: any two consecutive Title Case words not in the safe list
// gets its second word dropped, on the assumption it's more likely a
// surname than not. This can over-redact legitimate two-word phrases
// (add them to SAFE_TITLE_CASE_PAIRS as they come up) - that's the
...
function scrubFullNames(text) {
  return text.replace(/\b([A-Z][a-z]+) ([A-Z][a-z]+)\b/g, (match, first) =>
    SAFE_TITLE_CASE_PAIRS.has(match) ? match : first
  );
}
