...
async function buildDashboard() {
  ...
  // dropdown. Anything with "MKL" in the name is Maker Lab's own internal
  // entry, never a real client, so it's left out here regardless of what
  // EXCLUDE_COMPANIES is set to. ---
  const companyDirectory = clientCompanies
    ...
    .map((c) => ({
      ...
      contacts: contacts
        ...
        .map((ct) => ({ name: ct.name, lastContact: ct.lastContact }))
