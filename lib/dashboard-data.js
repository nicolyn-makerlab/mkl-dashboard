...
// Same pattern as chat - written twice a day by lib/granola-summary.js.
function readGranolaTopicsByCompany() {
  try {
...
async function buildDashboard() {
  ...
  const granolaTopicsByCompany = readGranolaTopicsByCompany();
  const companyDirectory = clientCompanies
    ...
    .map((c) => ({
      ...
      chatTopics: Object.prototype.hasOwnProperty.call(chatTopicsByCompany, c.name)
        ...
        : null,
      // null = no Granola note has matched this company yet; an array,
      // including empty, means the last run found (or didn't find) topics.
      execSummary: Object.prototype.hasOwnProperty.call(granolaTopicsByCompany, c.name)
        ? granolaTopicsByCompany[c.name]
        : null,
    }))
    .sort((a, b) => a.name.localeCompare(b.name));
