...
// The spreadsheet syncs from Caspio, which inserts a banner row (e.g.
// "Refreshed from Caspio daily") above the real headers on at least one
// tab - so "row 1 is the header row" doesn't always hold. Scan the first
// few rows of a tab instead of assuming any fixed row index.
const HEADER_SCAN_ROWS = 5;

function findHeaderRowIndex(rows, requiredCols) {
  for (let i = 0; i < Math.min(rows.length, HEADER_SCAN_ROWS); i++) {
    const row = rows[i] || [];
    if (requiredCols.every((c) => row.includes(c))) return i;
  }
  return -1;
}
...
//
// Returns { title, headerRowIndex } so the caller knows where the actual
// data starts, not just which tab - see HEADER_SCAN_ROWS above.
  ...
  for (const title of titles) {
    ...
    try {
      const res = await sheets.spreadsheets.values.get({
        spreadsheetId,
        range: `'${title.replace(/'/g, "''")}'!A1:E${HEADER_SCAN_ROWS}`,
      });
...
async function fetchTalentData() {
  ...
  const [contractsRes, rosterInfo] = await Promise.all([
  ...
  if (rosterInfo) {
    const rosterRes = await sheets.spreadsheets.values.get({
      spreadsheetId,
      range: `'${rosterInfo.title.replace(/'/g, "''")}'!A:E`,
    });
    const rows = rosterRes.data.values || [];
    const header = rows[rosterInfo.headerRowIndex] || [];
    const idIdx = header.indexOf("Employee ID");
    const startIdx = header.indexOf("Start Date");
    for (const row of rows.slice(rosterInfo.headerRowIndex + 1)) {
  ...
  const headerRowIndex = findHeaderRowIndex(rows, ["Client", "Terminated"]);
  if (headerRowIndex === -1) {
    throw new Error(
      `"Client Contracts" tab is missing a Client or Terminated column in its first ${HEADER_SCAN_ROWS} rows - check it hasn't been renamed. Rows found: ${JSON.stringify(rows.slice(0, HEADER_SCAN_ROWS))}`
    );
  }
